...
void SPI_Initial( void )
{
  /* 初始化SPI */
  /*
    高位在前 4分频（16/4Mhz）主机模式 空闲高电平  上升沿触发 双工模式 
    软件片选  开启crc7校验 
  */
        SPI_DeInit(SPI1);
	SPI_Init(SPI1,SPI_FirstBit_MSB, SPI_BaudRatePrescaler_4, SPI_Mode_Master,\
	 ...
	 SPI_Cmd(SPI1,ENABLE);	/* 使能SPI */
		...
		GPIO_Init( GPIOB, GPIO_Pin_5 | GPIO_Pin_7, GPIO_Mode_Out_PP_High_Slow );
}
...
void GPIO_Initial( void )
{
    ...
    GPIO_Init( GPIOD, GPIO_Pin_4|GPIO_Pin_5, GPIO_Mode_Out_PP_High_Slow );
		
    //config the IRQ0 and IRQ1 for Sx1212, IRQ0 = PB4, IRQ1 = PB3
    GPIO_Init( GPIOB, GPIO_Pin_4 | GPIO_Pin_3, GPIO_Mode_In_PU_No_IT );
}
